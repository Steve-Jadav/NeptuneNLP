...
class Classifier_LSTM():
    ...
    def get_class_name(self, row):
        return self.class_rev_map[row]

    def get_tokenizer(self, save = False):
        tokenizer = Tokenizer()
    ...
    def get_padded_sent(self, tokenizer):
    ...
    def get_glove_embeddings(self, tokenizer):
    ...
    def train_on_data(self, save = False, graphs = False):

        tokenizer = self.get_tokenizer()
        embedding_matrix = self.get_glove_embeddings(tokenizer)
        padded_sent = self.get_padded_sent(tokenizer)

        Y = self.df['class'].apply(self.assign_category)
        Y = to_categorical(Y, 12)
        model = self.get_model(embedding_matrix)
    ...
    def predict(self):
        ...
        self.df['predict'] = self.df['predict'].apply(self.get_class_name)
